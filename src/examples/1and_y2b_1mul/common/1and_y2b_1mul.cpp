...
int32_t test_millionaire_prob_circuit(e_role role, const std::string& address, uint16_t port, seclvl seclvl,
	...
	if(role == SERVER) {
		s_alice_money = yc->PutDummyINGate(bitlen);
		s_bob_money = yc->PutINGate(bob_money, bitlen, SERVER);
	} else { //role == CLIENT
		s_alice_money = yc->PutINGate(alice_money, bitlen, CLIENT);
		s_bob_money = yc->PutDummyINGate(bitlen);
	}
...
share* BuildMillionaireProbCircuit(share *s_alice, share *s_bob, Circuit *ac, Circuit *bc, Circuit *yc) {
    ...
    and_out_y = yc->PutANDGate(s_alice, s_bob);
	and_out_b = bc->PutY2BGate(and_out_y);
	mul_out_b = bc->PutMULGate(and_out_b, and_out_b);

	return bc->PutOUTGate(and_out_b, ALL);
}
