...
int32_t test_circuit(e_role role, char* address, uint16_t port, seclvl seclvl,
	...
	*/
	...
	for(int i = 0; i < num_tests; i++) {
		if(role == SERVER) {
			s_alice_input[i] = bc->PutDummyINGate(bitlen);
			s_bob_input[i] = bc->PutINGate((bitlen == 64 ? (uint64_t)bob_input[i]: bob_input[i]), bitlen, SERVER);
		} else { //role == CLIENT
			s_alice_input[i] = bc->PutINGate((bitlen == 64 ? (uint64_t)alice_input[i]: alice_input[i]), bitlen, CLIENT);
			s_bob_input[i] = bc->PutDummyINGate(bitlen);
		}
		...
		*/
		...
		s_out1[i] = ac->PutB2AGate(s_alice_input[i]);
		s_out2[i] = ac->PutB2AGate(s_bob_input[i]);
		...
		*/
		s_out1[i] = ac->PutOUTGate(s_out1[i], ALL);
		s_out2[i] = ac->PutOUTGate(s_out2[i], ALL);
	...
	*/
	...
	std::cout << "\nTesting " << num_tests << " B2A operations ";
