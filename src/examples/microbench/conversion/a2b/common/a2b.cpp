	...
	*/
	...
	for(int i = 0; i < num_tests; i++) {
		...
		*/
		...
		s_out1[i] = bc->PutA2BGate(s_alice_input[i], yc);
		s_out2[i] = bc->PutA2BGate(s_bob_input[i], yc);
		...
		*/
		s_out1[i] = bc->PutOUTGate(s_out1[i], ALL);
		s_out2[i] = bc->PutOUTGate(s_out2[i], ALL);
