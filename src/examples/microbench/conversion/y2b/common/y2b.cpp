...
int32_t test_circuit(e_role role, char* address, uint16_t port, seclvl seclvl,
	...
	*/
	...
	for(int i = 0; i < num_tests; i++) {
		if(role == SERVER) {
			...
			s_bob_input[i] = yc->PutINGate((bitlen == 64 ? (uint64_t)bob_input[i]:bob_input[i]), bitlen, SERVER);
		} else { //role == CLIENT
			s_alice_input[i] = yc->PutINGate((bitlen == 64 ? (uint64_t)alice_input[i]:alice_input[i]), bitlen, CLIENT);
		...
		*/
		s_out[i] = bc->PutOUTGate(s_out[i], ALL);
	...
	*/
	...
	std::cout << "\nTesting " << num_tests << " Y2B operations ";
