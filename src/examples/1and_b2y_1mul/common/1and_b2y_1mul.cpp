...
int32_t test_millionaire_prob_circuit(e_role role, const std::string& address, uint16_t port, seclvl seclvl,
	...
	if(role == SERVER) {
		s_alice_money = bc->PutDummyINGate(bitlen);
		s_bob_money = bc->PutINGate(bob_money, bitlen, SERVER);
	} else { //role == CLIENT
		s_alice_money = bc->PutINGate(alice_money, bitlen, CLIENT);
		s_bob_money = bc->PutDummyINGate(bitlen);
	}
...
share* BuildMillionaireProbCircuit(share *s_alice, share *s_bob, Circuit *ac, Circuit *bc, Circuit *yc) {
    ...
    and_out_b = bc->PutANDGate(s_alice, s_bob);
	and_out_y = yc->PutB2YGate(and_out_b);
	mul_out_y = yc->PutMULGate(and_out_y, and_out_y);

	return yc->PutOUTGate(mul_out_y, ALL);
}
